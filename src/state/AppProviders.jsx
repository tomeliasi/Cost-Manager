...
/**
 * Centralize DB + Rates handling to keep pages lean.
 * - DB: IndexedDB (required)
 * - Rates: fetched from URL (required) with default URL if user didn't set it.
 */
export function AppProviders({ children }) {
  ...
  useEffect(() => {
    ...
    openCostsDB(DB_NAME, DB_VERSION)
      .then((repo) => { if (alive) setDb(repo); })
      .catch((e) => { if (alive) setDbError(String(e?.message || e)); });
  ...
  useEffect(() => {
    ...
    fetchRates(settings.ratesUrl)
      ...
      .catch((e) => { if (alive) setRatesError(String(e?.message || e)); });
